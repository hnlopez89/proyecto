...
async function listBusinessTodayBookings(req, res, next) {
  ...
  try {
    ...
    const [result] = await connection.query(
      `SELECT B.check_in_time, check_in_day, B.check_out_time, B.check_out_day, users.name, users.surname, users.email, B.units, B.status, B.creating_date, B.update_date
            FROM booking B
